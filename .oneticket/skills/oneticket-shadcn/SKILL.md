...
description: shadcn/ui components, theming, forms — conventions for projects using shadcn/ui with Vite (not Next.js).
source: external
...
These rules are **always enforced**. Each links to a reference with Incorrect/Correct code pairs.
...
These are the most common patterns that differentiate correct shadcn/ui code. For edge cases, see the rule sections above.
...
## Key Fields (Vite-specific)

The injected project context contains these key fields:

- **`aliases`** → use the actual alias prefix for imports (e.g. `@/`, `~/`), never hardcode.
- **`isRSC`** → when using Vite, this is typically `false` (no React Server Components). Always reference this field when advising on component usage.
...
- **`framework`** → for Vite projects, this will be `vite` or `react-router`. Never assume Next.js routing.
...
1. **Get project context** — run `npx shadcn@latest info --json` to get the current configuration.
...
9. **Switching presets** — Ask the user first: **overwrite**, **partial**, **merge**, or **skip**?
   ...
   - **Partial**: `npx shadcn@latest apply <code> --only theme,font`. Updates only the selected preset parts without reinstalling UI components. Supported values are `theme` and `font`; comma-separated combinations are allowed.
...
```bash
# Initialize existing Vite project with shadcn/ui.
npx shadcn@latest init --preset base-nova --template vite
...
For full documentation, visit [https://ui.shadcn.com/docs](https://ui.shadcn.com/docs).

## Source

This skill is sourced from [shadcn/ui](https://github.com/shadcn/ui) and adapted for Vite projects (not Next.js).
