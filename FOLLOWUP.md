# OneTicket v1.0.0 — Implementation Followup

## Authoring rules

This document is a memory aid for humans — no details, no specs, no explanations.
One line per item. All detail lives in `product-spec.md`, `AGENTS.md`, or the code.

## Backlog

- [`product-spec.md`](.oneticket/docs/what/product-spec.md) — precision pass on existing chapters
- [`product-spec.md`](.oneticket/docs/what/product-spec.md) — new UX chapter based on [`oneticket-brief.md`](.oneticket/docs/what/oneticket-brief.md)
- Setup APM

## In Progress

## Done

- Archive v0.5.0 code to `*.old` — open v1.0.0 workspace
  - `src/` → `src.old/`
  - `.github/workflows/` → `.github/workflows.old/`
  - `.oneticket/agents/` → `.oneticket/agents.old/`
  - `.oneticket/skills/` → `.oneticket/skills.old/`

## Decisions log

## Open questions

