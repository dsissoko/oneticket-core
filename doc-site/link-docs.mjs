...
// Sidebar order map — controls sort order in Starlight autogenerated sidebar.
// Keys are relative paths from destDir (post-copy). Values are sidebar.order numbers.
// Lower numbers appear higher in the sidebar.
// Only define order where alphabetical sort is wrong — epics/us use numeric prefixes
...
const SIDEBAR_ORDERS = {
  ...
  // how/ level 2: architecture first, then c4
  'how/architecture.md':       1,
  'how/c4/index.md':           2,
  // how/c4/: canonical C4 order
  'how/c4/system-context.md':  1,
  'how/c4/containers.md':      2,
  'how/c4/components.md':      3,
...
function transformMarkdown(content, srcFile) {
  ...
  const relPath  = path.relative(destDir, srcFile).replace(/\\/g, '/');
  const order    = SIDEBAR_ORDERS[relPath];
  const sidebarFm = order !== undefined ? `\nsidebar:\n  order: ${order}` : '';
  return `---\ntitle: "${title.replace(/"/g, '\\"')}"${sidebarFm}\n---\n${converted}`;
}
...
function generateIndex(dir) {
  ...
  const relPath   = path.relative(destDir, path.join(dir, 'index.md')).replace(/\\/g, '/');
  const order     = SIDEBAR_ORDERS[relPath];
  const sidebarFm = order !== undefined ? `\nsidebar:\n  order: ${order}` : '';

  fs.writeFileSync(
    path.join(dir, 'index.md'),
    `---\ntitle: '${title}'${sidebarFm}\n---\n\n${links.join('\n')}\n`
  );
}
